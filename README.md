...
В отчёте есть все три. Обход идёт по самому широкому из них, что укладывается в
...
возвращается обычно именование провайдера, а не сами сайты.
...
**Разрешает каждого кандидата прямым запросом.** Именно этот шаг отличает reverse-IP
...
Соседом называются только первые два. Остальные вынесены в отдельную таблицу — обычно
она интереснее.
...
* **Покрытие.** Корпоративный /24 с PTR и сертификатами — почти целиком. Типичный
  ...
  какие источники ответили, вместо намёка, что увидено всё.
