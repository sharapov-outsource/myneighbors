...
OWN.ru = {
  ...
  h1: 'Соседи по IP',
  subtitle: 'Всё остальное, что живёт на адресе и в подсети вокруг него: обратный DNS, имена в сертификатах, реестр и таблица маршрутизации — и каждое имя проверено прямым запросом, прежде чем названо соседом',
  ph_host: '8.8.8.8',
  hero_label: 'Проверяемый адрес',
  hero_count: 'сайтов здесь',
  badge_cdn: 'узел CDN',
  empty_hint: 'Введите IP-адрес, подсеть в виде 8.8.8.0-24 или доменное имя. Проверка сначала выясняет, в какой блок входит адрес — выделение из реестра, анонсируемый префикс и /24 совпадают редко, — затем читает обратный DNS для каждого адреса блока и сертификат, который каждый из них показывает, когда сайт не назван. Каждое найденное имя проверяется прямым запросом: в сертификате перечислены имена, на которые он выписан, а не те, что здесь всё ещё живут.',

  /* ---- stages ---- */
  stage_resolve: 'определяем адрес',
  stage_block: 'выясняем блок',
  ...
  k_block: 'Проверенный блок',
  k_block_source: 'Выбран потому что',
  k_range: 'Диапазон',
  k_swept: 'Пройдено адресов',
  k_with_ptr: 'С обратным DNS',
  k_responded: 'Ответили на 443',
  k_net_name: 'Имя сети',
  k_holder: 'Владелец',
  k_net_range: 'Выделение',
  k_country: 'Страна',
  k_rir: 'Регистратура',
  k_registered: 'Зарегистрирован',
  ...
  /* ---- empty states ---- */
  empty_here: 'Ни одно имя не разрешается в этот адрес. Обычный ответ для инфраструктуры — резолвера, почтового релея, NAT-шлюза — и для сайта, спрятанного за прокси.',
  empty_cdn: 'Это узел CDN. На нём отвечают миллионы никак не связанных сайтов, так что полезного ответа у вопроса здесь нет.',
  ...
  /* The shared wording withholds a grade; this service does not have one. */
  incomplete_body: 'Часть проверок не получила ответа, и ничего ниже не додумано, чтобы закрыть пробел. Чего не хватает:',

  /* ---- notes ---- */
  note_address: 'Считаются только имена, которые разрешаются в этот адрес прямо сейчас. Перечисленное в сертификате — утверждение о моменте выпуска.',
  note_block: 'Проходится самый широкий блок, который умещается в лимит обхода. Более широкие блоки описаны ниже, но не проходятся.',
  note_routing: 'Анонсируемый префикс взят из глобальной таблицы маршрутизации через Team Cymru и обычно шире выделения из реестра.',
  note_here: 'Источник — то, где имя встретилось впервые. Подтверждение — свежий запрос A или AAAA, сделанный уже после.',
  note_neighbors: 'Одно TCP-соединение на адрес, порт 443, без указания сайта — именно поэтому сервер показывает сертификат по умолчанию.',
  note_candidates: 'Три разных ответа на «та же подсеть», и ни один не ошибочный. Реестр говорит, чей это адрес, маршрутизация — чья сеть его несёт, а /24 — это единица, которой торгует хостинг.',
  ...
  /* ---- values ---- */
  v_this_one: 'о нём и спрашивали',
  v_uncountable: 'не сосчитать',

  /* ---- confirmation status ---- */
  nst_confirmed: 'обслуживается здесь',
  nst_in_block: 'в другом месте блока',
  nst_moved: 'переехал',
  nst_unresolved: 'больше не разрешается',
  nst_wildcard: 'wildcard, разрешать нечего',
  ...
  bsrc_requested: 'вы его и запросили',
  ...
  inc_sweep: 'блок пройти не удалось',
  inc_certificates: 'сертификаты не собирались',
  inc_reverse_ip: 'ни один reverse-IP индекс не ответил',
  inc_verification_truncated: 'кандидатов больше, чем лимит проверки',

  /* ---- errors of this service ---- */
  err_invalid_prefix: 'Длина префикса должна быть числом от 0 до размера адресного пространства.',
  err_block_too_large: 'В этом блоке больше адресов, чем можно пройти за одну проверку. Запросите более узкий префикс.',
  ...
  fd_shared_address: 'Прямо сейчас сюда разрешается больше трёх имён. Это шаред-хостинг или обратный прокси, и всё, что на нём стоит, делит репутацию адреса, общий рейт-лимит и любую блокировку, которую кто-то из соседей заработает.',

  flag_no_names_confirmed: 'Ни одно имя сюда не разрешается',
  fd_no_names_confirmed: 'Ничего из найденного не удалось подтвердить через DNS. Так выглядит инфраструктура, которую никто не называет по имени, адрес за прокси и адрес, которого нет в индексах.',

  flag_stale_claims: 'Большая часть заявленного переехала',
  fd_stale_claims: 'Источники называют больше уехавших сайтов, чем оставшихся. Так ведут себя и старые сертификаты, и несвежие reverse-IP индексы — стоит знать об этом, прежде чем цитировать число из любого из них.',

  flag_no_reverse_dns: 'Во всём блоке нет обратного DNS',
  fd_no_reverse_dns: 'Ни один адрес блока не отвечает на PTR-запрос. Обычное дело для облачных диапазонов, которые выдаются поминутно, и это лишает самого дешёвого способа понять, кто чем владеет.',
  ...
  flag_block_wider_than_sweep: 'Настоящий блок шире пройденного',
  fd_block_wider_than_sweep: 'Реестр или таблица маршрутизации помещают адрес в диапазон больше того, что можно пройти за одну проверку, так что перечисленные соседи — его часть, а не весь список.',

  flag_block_not_enumerable: 'Блок IPv6 пройти нельзя',
  fd_block_not_enumerable: 'В /64 восемнадцать квинтиллионов адресов — поэтому сканирования адресов в IPv6 не существует. Показать можно только то, что уже знают индексы и логи сертификатов.',

  flag_block_too_large: 'Блок слишком велик для обхода',
  fd_block_too_large: 'В блоке с этим адресом больше адресов, чем допускает одна проверка, поэтому он сужен до лимита обхода.',

  flag_probing_disabled: 'Сертификаты не собирались',
  fd_probing_disabled: 'Активный шаг выключен, и отчёт собран только из обратного DNS и публичных индексов. Это более тихая половина — и менее полная.',

  flag_no_reverse_ip_source: 'Ни один reverse-IP индекс не ответил',
  fd_no_reverse_ip_source: 'Имена не пришли ниоткуда, кроме самой проверки, — значит, всё, что живёт здесь без сертификата и без обратного DNS, для неё невидимо.',

  flag_providers_available: 'Можно подключить больше источников',
  fd_providers_available: 'У этой установки есть reverse-IP провайдеры, для которых не задан ключ API. С настроенным ключом покрытие шаред-хостинга заметно вырастет.',
};
